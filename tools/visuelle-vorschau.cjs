...
function Sichtprobe(){
 ...
 // Die Wildcard-Sichtprobe nimmt je Seltenheit die aktuell längste reale Kombination
 // aus Titel + Beschreibung. So kann ein kurzer Dummytext keine mobile Überlänge verdecken.
 const wildcardProbe=r?WILDCARDS.filter(w=>w.r===r).reduce((best,w)=>{
  const laenge=x=>String(x.n||'').length+String(x.t||'').length;
  return !best||laenge(w)>laenge(best)?w:best;
 },null):null;
 ...
 {modus==='wildcards'&&<><p>Aufdeckung starten:</p>{Object.keys(RARITY).map(k=><button className="btn" key={k} onClick={()=>{setR(k);setLauf(v=>v+1)}}>{k}</button>)}{wildcardProbe&&<WildcardEnthuellung key={lauf} card={wildcardProbe} onFertig={()=>setR(null)}/>}</>}
 {modus==='spielerkarten'&&<><button className="btn" onClick={()=>setLauf(v=>v+1)}>Neu aufdecken</button><div style={{maxWidth:400,margin:'20px auto',display:'grid',gap:24}}>{['bronze','silber','gold','legende'].map((st,i)=><Spielerkarte key={st+lauf} karte={{kid:'probe'+i,stufe:st,name:'Mika Hartmann',nat:'GER',flag:'🇩🇪',pos:'ZM',ovr:55+i*12,alter:22,herkunft:'pack'}} gross jubel/>)}{['gold','legende'].map((st,i)=><Spielerkarte key={'halle'+st+lauf} karte={{kid:'halleprobe'+i,stufe:st,name:i?'Laura Lange':'Collin Mullinster',nat:i?'GER':'IRL',flag:i?'🇩🇪':'🇮🇪',pos:i?'ST':'AF',ovr:90+i,alter:38+i*2,herkunft:'halle',zusatz:{titel:5}}}/>)}</div></>}
...
`;
 ...
 fs.writeFileSync(path.join(out,'sichtprobe.html'),'<!doctype html><html lang="de"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Rasenschach Sichtprobe</title><style>'+fs.readFileSync(path.join(root,'charakter-ui.css'),'utf8')+'</style><body><div id="root"></div><script>'+result.outputFiles[0].text.replace(/<\/script/gi,'<\\/script')+'</script></body></html>');console.log(path.join(out,'sichtprobe.html'));
})();
