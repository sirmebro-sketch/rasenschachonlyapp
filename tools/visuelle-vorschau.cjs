...
function Sichtprobe(){
 ...
 <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(145px,1fr))',gap:16}}>{opts.map(f=><div key={f}><Avatar size={145} g={geschlecht} meta={meta} zuege={{...zuegeAusKennung(1,geschlecht,'GER',meta),stil:2,kopf,haar,haut:hautton,frisur:1,bart:0,schmuck:0,schminke:0,details:0,[merkmal]:f}}/><p style={{fontSize:12}}>{f}: {merkmal==='frisur'?(f>=(geschlecht==='w'?14:16)?NEUE_FRISUREN[f-(geschlecht==='w'?14:16)]:FRISUR_NAMEN[geschlecht][f]):merkmal==='kopf'?KOPFFORM[f]?.n:merkmal==='augenfarbe'?AUGENFARBE[f]?.n:PORTRAET_NAMEN[merkmal]?.[f]}</p></div>)}</div></>}
