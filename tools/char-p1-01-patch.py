...
old = '''  { n: "Zart",     b: 24,   j: 15, kinn: 70, kv: .58 },
  { n: "Rundlich", b: 27.5, j: 21, kinn: 68, kv: .58 },
];
/* Kieferbreite AM KINN.'''
new = '''  { n: "Zart",     b: 24,   j: 15, kinn: 70, kv: .58 },
  { n: "Rundlich", b: 27.5, j: 21, kinn: 68, kv: .58 },
...
/* Kieferbreite AM KINN.'''
if old not in s:
    raise SystemExit('KOPFFORM-Anker nicht gefunden')
s = s.replace(old, new, 1)

old_a = '''  const A = ZUEGE_ANZAHL(meta, w);
  const z = {};
  ZUEGE_ORDNUNG.forEach((k, i) => { z[k] = mische(h, i) % A[k]; });'''
new_a = '''  /* Neue Editor-Kopfformen dürfen die deterministische Ableitung alter
     Porträts nicht verändern: historisch waren hier genau zehn Kopf-IDs. */
  const A = { ...ZUEGE_ANZAHL(meta, w), kopf: KOPFFORM_HISTORISCH };
  const z = {};
  ZUEGE_ORDNUNG.forEach((k, i) => { z[k] = mische(h, i) % A[k]; });'''
if old_a not in s:
    raise SystemExit('zuegeAusKennung-Anker nicht gefunden')
s = s.replace(old_a, new_a, 1)

start = s.index('const kopfPfad = (k) => {')
end = s.index('\n};', start) + 3
old_path = s[start:end]
new_path = '''const kopfPfad = (k) => {
...
if 'return "M" + l + ",40 C"' not in old_path:
    raise SystemExit('kopfPfad-Anker unerwartet')
s = s[:start] + new_path + s[end:]
p.write_text(s)
print('App.jsx erfolgreich gepatcht')
