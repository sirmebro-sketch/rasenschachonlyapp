...
test('Spielerkarten: Gold wirkt metallisch, Legendaer bleibt irisierend',async({page},testInfo)=>{
 ...
 await expect(gold).toHaveCSS("color","rgb(36, 27, 9)");
 await expect(gold.locator(".eb").first()).toHaveCSS("color","rgb(36, 27, 9)");
 ...
 await expect(gold).toHaveCSS('color','rgb(36, 27, 9)');
 await expect(gold).toHaveCSS('background-image',/rgb\(235, 204, 119\)/);
});
