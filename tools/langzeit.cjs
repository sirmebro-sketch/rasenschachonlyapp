...
try{
 ...
 for(const pos of Object.keys(E.POS))for(const gender of ['m','w'])for(const mode of E.MODES)for(let choice=0;choice<4;choice++){
  ...
  try{
   ...
   rows.push({pos,gender,mode:mode.id,years:r.p.seasons.length,vc:r.vc,events:r.logs.length,ids:r.logs,jahre:r.jahre,vorsatz:r.vorsatz?.id,erreicht:r.vorsatz?.erreicht,lohnSaison:r.p.vorsatzLohn?.[r.vorsatz?.id]?.saisons});
 ...
 const goals=['welt','daheim','lange','glanz','beruf','einsatz'].map(id=>{const a=rows.filter(r=>r.vorsatz===id);return {id,n:a.length,erfuellt:a.filter(r=>r.erreicht).length,quoteProzent:+(100*a.filter(r=>r.erreicht).length/a.length).toFixed(1),belohnungSaison:a.filter(r=>r.lohnSaison!=null).map(r=>r.lohnSaison)};});
