...
export function KartenEffekt({form='karte',stark=false,still=false,hsv=false,dezent=false}){
 ...
 // 23.09.2026: "dezent" wird nur von Gold-/Legendär-Spielerkarten benutzt.
 // 16 % Gesamtdeckkraft aus 35.195.1 multiplizierte selbst die kräftigsten
 // Farb-Stopps auf rund 5 % herunter. Der Inhalt bleibt durch den negativen
 // Stapelindex frei; deshalb darf die Materialfarbe wieder sichtbar tragen.
 const deckkraft=dezent?(stark?.78:.66):1;
  ...
  <defs>
   ...
   <linearGradient id={id+'holo'} x1="0" y1="0" x2="1" y2=".65">
    {dezent ? <>
     ...
     <stop offset=".17" stopColor="#ff6ecb" stopOpacity={stark?'.50':'.40'}/>
     <stop offset=".34" stopColor="#8f7dff" stopOpacity={stark?'.52':'.42'}/>
     <stop offset=".52" stopColor="#55e8ff" stopOpacity={stark?'.48':'.40'}/>
     <stop offset=".70" stopColor="#77ffad" stopOpacity={stark?'.44':'.36'}/>
     <stop offset=".86" stopColor="#ffd76d" stopOpacity={stark?'.46':'.38'}/>
